...
<!-- <details>
    <summary style="font-weight: bold; text-decoration: underline; cursor: pointer;">💥 capitulo 2 -gestion de estados💥</summary> -->

- # Capítulo 2 - Gestión de Estados

  ## Introduccion

  - **Antes:** Se utilizaban principalmente `useState` para realizar llamados de datos del servidor y gestionar estados del cliente.
  - **Ahora:** Se utiliza `React-query` para la gestión del estado del `servidor`, mientras que para lo que el `cliente` está haciendo se usa `context`.

  ### Cuándo Usar Context

  El `context` se usa para la gestión de estados relacionados con acciones específicas que el usuario realiza. Por ejemplo:

  - **El usuario actual que se conecta.**
  - **Filtros aplicados.**
  - **El tema que se usa (modo claro/oscuro).**

  Es decir, solo se debe utilizar para manejar lo que el `usuario esté utilizando en ese momento`. En estos casos, se recomienda usar `useState` o `useReducer`. Para compartir estos estados con componentes más abajo en el árbol de componentes, se utiliza `context`.

  ### Nota Importante

  - **IMPORTANTE:** Si metemos todos los estados en el `context`, tendremos un `mega estado`. Esto puede causar que, al cambiar un estado, todos los `componentes` conectados con ese `context` se rendericen, incluso si no están relacionados con el cambio.

    - se podria dividir los context, pero se empezaria a tener `context1, contex2 ....contextn` lo cual ocasionaria la `piramide de la muerte` que significa tener un descontrol y poco mantenible

  - `SOLUCION:` Se recurre entonces a una biblioteca, ya sea , `Redux, Mobx, xState, Zustand`

  # 1) creacion de un context

  creo mi archivo y mi proyecto de react:
  ...
  #### ubicacion: `reactUpdates24/2-Gestion_estados/1-context`

  - ## 1.1) creacion sin context inicial

    creo el elemento principal y dos componetes hijos los cuales uno tendra dentro de si otro componente y a ese se le pasara nuevamente el componente, esto con el fi e ver que pasa si no se usa context

    #### u: `reactUpdates24/2-Gestion_estados/1-context/src/App.tsx`
    ...
    #### u `reactUpdates24/2-Gestion_estados/1-context/src/components/MainContent.tsx`
    ...
    function MainContent({ todos, addTodo }: Props) {
      return (
        <div>
          ...
          {/*-------------------------------le paso nuevamente las propiedades lo cual empieza a resultar muy diicil de mantener a largo plaso*/}
    ...
    Creo el compoente Todolist

    #### u `reactUpdates24/2-Gestion_estados/1-context/src/components/TodoList.tsx`
    ...
    //------------aqui recibe nuevamente las propiedades, lo cual viene desde el componente padre y lo cual resulta un codigo dificil de mantener
  ...
  - ## 1.2) creacion ahora si del contexto para evitar estar pasando por muchos componetes

    - creo la carpeta: `reactUpdates24/2-Gestion_estados/1-context/src/components/contexts`
    - creo el archivo: `reactUpdates24/2-Gestion_estados/1-context/src/components/contexts/TodosContext.ts`
  ...
  - ### 1.2.3) Envolviendo la aplicacion con el context

    - #### u : `reactUpdates24/2-Gestion_estados/1-context/src/App.tsx`

      ahora si hacemos uso del context o envolvemos a la aplicacioncon el context:
    ...
    Ahora voy al elemento `Dashboard y MainContent y TodoList` que deseo usar el context

    - **Dashboard**
    - #### u : `reactUpdates24/2-Gestion_estados/1-context/src/components/Dashboard.tsx`

    Agregamos el context y quitamos las propiedades
